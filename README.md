...
- 🍎 **菜单栏常驻** —— 顶部菜单栏图标，状态实时显示
- 🪟 **图形化设置面板** —— 可视化配置 API Key、权限和 Prompt，无需编辑配置文件
- ⌨️ **多快捷键绑定** —— 支持同时配置多个快捷键，每个对应不同的 Prompt 处理模式
- ✏️ **可视化 Prompt 编辑** —— 设置窗口中直接添加/编辑/删除快捷键绑定
- 🛡️ **自动权限引导** —— 启动时自动检测并请求所需权限
- 🤖 **AI 智能整理** —— 基于 DeepSeek，自动纠错、优化、Markdown 结构化
- 🎯 **自动全选** —— 无需手动选中文本，按一下就处理
- 🎨 **Lottie 动效提示** —— 处理过程中屏幕顶部居中透明浮层动画
...
完成后，菜单栏图标显示「● 运行中」，光标停在任意输入框内，**按配置的快捷键**（默认右 Option）即可处理当前文本。

### 配置多快捷键和 Prompt

在设置窗口的「快捷键绑定」区域：

1. **点击「添加绑定」** 创建新的快捷键-Prompt 配对
2. **选择快捷键**（右 Option、左 Option、右 Cmd 或组合键）
3. **输入 Prompt 名称和内容**（完整的 AI 指令）
4. **点击「保存」**

示例配置：
- **右 Option** → 智能结构化（自动识别类型，转 Markdown）
- **右 Cmd** → 快速纠错（仅修正错别字和语法）
- **Cmd+Shift+P** → 专业扩展（添加细节和背景信息）

每个快捷键独立工作，按下对应的键即可触发相应的 AI 处理。
...
打开「设置…」窗口，在「快捷键绑定」区域：
- 添加、编辑、删除快捷键绑定
- 每个绑定包含：快捷键、Prompt 名称、Prompt 内容
- 保存后自动生效（建议重启服务）

### 手动编辑配置文件（高级）

配置文件位于 `~/.voice-text-enhancer/config.yaml`：
...
**支持的快捷键：**
- 单键：`right_option`, `left_option`, `right_cmd`
- 组合键：`<cmd>+<shift>+p`, `<cmd>+<alt>+space` 等

**向后兼容：** 旧格式 (`hotkey.trigger` + `active_prompt`) 仍然支持，会自动转换为新格式。
